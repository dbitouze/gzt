...
\begin{dbwarning}{Codage d'entrée \protect\lstinline+UTF-8+ obligatoire}{}
  ...
  \item elle suppose que les fichiers source \file{.tex} sont effectivement
...
\begin{bodycode}
\question{Ton sujet principal est celui des systèmes dynamiques, mais quels sont
  les autres thèmes qui t'intéressent ?}
...
\begin{dbwarning}{Insertion manuelle d'espacements inutile}{}
  ...
  manuellement des espacements horizontaux ou verticaux (\docAuxCommand*{hspace},
  \docAuxCommand*{hskip}, \docAuxCommand*{vspace}, \docAuxCommand*{vskip}, etc.).
\end{dbwarning}
...
\section{Outils communs à tous types d'articles}
\label{sec-outils-communs-tout}
...
\end{docKey}
\begin{docKey}{label}{=\meta{label}}{pas de valeur par défaut,
    initialement vide}
  Cette clé permet de spécifier l'éventuel \meta{label} de l'image, de sorte
  à y faire référence, notamment au moyen de la commande \docAuxCommand*{vref} du
  \Package{varioref}.
\end{docKey}
...
%
\begin{gztfigure}[title=Courbe elliptique correspondant à l'équation $y^2=x^3-x$,label=elliptique]
...
recrutées est [...]
%
\begin{gzttable*}[label=recru-mc-25,title=Recrutement des maîtres de conférences section 25]
...
\end{gzttable*}
%
\begin{gzttable*}[label=recru-mc-26,title=Recrutement des maîtres de conférences section 26]
\begin{tabular}{|l|cccccc|}
  \hline
  & 2008 & 2009 & 2010 & 2011 & 2012 & 2013 \\ \hline
  [...]
...
\item À la fin du document, faire figurer la liste des références
  bibliographiques au moyen de la commande \refCom{printbibliography} suivante.
  \end{enumerate}
...
\item charge les packages :
  ...
  \item \package*{mathtools} qui lui-même charge \package*{amsmath}
    (mais en l'étendant et en corrigeant certains de ses défauts), si bien que
    toutes les commandes de ces deux packages sont disponibles ;
...
\paragraph{Fonctions trigonométriques circulaires et hyperboliques, directes et réciproques}

\begin{docCommand}{cotan}{}
  Cette commande affiche la cotangente :
  \enquote{$\cotan$}.
\end{docCommand}
...
\begin{docCommand}{ch}{}
  Cette commande affiche le cosinus hyperbolique :
  \enquote{$\ch$}.
\end{docCommand}

\begin{docCommand}{sh}{}
  Cette commande affiche le sinus hyperbolique :
  \enquote{$\sh$}.
\end{docCommand}
...
\begin{docCommand}{tanh}{}
  ...
  \begin{dbremark}{Pas de commande \protect\lstinline+\\th+ pour la tangente hyperbolique}{}
...
\item[Propositions] \
  \begin{docEnvironment}{proposition}{}
    Cet environnement permet de composer une proposition. Son titre récurrent est
    \enquote{Proposition}.
  \end{docEnvironment}
  %
\item[Lemmes] \
...
\begin{docKey}{listing and text}{}{}
  ...
  \end{itemize}
  %^^A
  \begin{dbwarning}{Option \protect\refKey*{listing and text} : seulement pour du
      code \LaTeX{}}{}
...
L'environnement \verb|eqnarray| est obsolète et on utilisera plutôt \verb|align| :
...
L'exemple suivant illustre l'usage de cet environnement\conseilslatex{}. Le code :
...
\begin{docKey}{listing options}{}{}
  ...
  toute option connue du \Package*{listings}, notamment le langage sous-jacent au
  moyen de l'option \docAuxKey{language}.
\end{docKey}
...
\gztverb{\rm} sont désormais à proscrire. Elles doivent être remplacées par les
commutateurs, respectivement \gztverb{\bfseries}, \gztverb {\itshape},
