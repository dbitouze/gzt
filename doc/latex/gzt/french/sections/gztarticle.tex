...
\begin{itemize}
\item spécifiques aux images et tableaux :
  \begin{itemize}
  \item \refEnv{gztfigure}, \refEnv{gztfigure*}, \refEnv{gztfigure-},
    \refEnv{gztfigure*-} ;
  \item \refEnv{gzttable}, \refEnv{gzttable*}, \refEnv{gzttable-},
    \refEnv{gzttable*-} ;
  \end{itemize}
...
\refEnv{gztfigure*} (ou leurs analogues \refEnv{gztfigure-} et
...
\begin{docEnvironment}[doclang/environment content=image incluse avec \docAuxCommand{includegraphics}]{gztfigure-}{\oarg{option(s)}}
  Cet environnement est analogue à \refEnv{gztfigure} sauf que la légende
  correspondante ne contient :
\begin{itemize}
\item que l'intitulé \enquote{Figure} et le numéro de la figure ;
\item notamment pas de titre (même si spécifié au moyen de la clé \refKey{title}).
\end{itemize}
\end{docEnvironment}
...
\refEnv{gzttable*} (ou leurs analogues \refEnv{gzttable-} et
...
\begin{docEnvironment}[doclang/environment content=tableau créé avec \docAuxEnvironment{tabular}]{gzttable-}{\oarg{option(s)}}
  Cet environnement est analogue à \refEnv{gzttable} sauf que la légende
  correspondante ne contient :
\begin{itemize}
\item que l'intitulé \enquote{Tableau} et le numéro du tableau ;
\item notamment pas de titre (même si spécifié au moyen de la clé \refKey{title}).
\end{itemize}
\end{docEnvironment}
...
Le contenu\footnote{Notamment des images et, le cas échéant, des tableaux.} devant être
encadré le sera au moyen de l'un des deux environnements \refEnv{gztframe} et
\refEnv{gztframe*} suivants.
...
\begin{docEnvironment}[doclang/environment content=contenu à encadrer]{gztframe}{\oarg{option(s)}}
  ...
  \item dans un cadre
    ...
    \item dépourvu de zone de titre (même si un titre est spécifié au moyen de
      la clé \refKey{title}).
    \end{itemize}
  \end{itemize}
\end{docEnvironment}
